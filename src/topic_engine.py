...
SEEDS = ["How to design a high-converting landing page that gets more leads", "Responsive web design checklist that prevents mobile visitors from leaving", "7 website homepage mistakes that quietly lose customers", "How to improve website speed without breaking the design", "Accessible website design: keyboard and contrast checklist", "How to structure a small business website that builds trust", "CSS Grid vs Flexbox: which one should you use and why", "Website redesign process from audit to launch", "How to design mobile navigation people can use instantly", "Website SEO basics for a new business that needs local customers"]
...
def replenish(min_pending=20, batch=50):
    existing=load("topics",[]); pending=sum(x.get("status")=="pending" for x in existing)
    if pending >= min_pending: return existing
