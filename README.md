...
Gemini generation uses an ordered model fallback list with transient-error retries and runtime discovery. The workflow prioritizes `gemini-3.6-flash` and `gemini-3.5-flash-lite`, then discovers other API models that support `generateContent` and appends them as fallbacks. Each model is retried for temporary 429/5xx capacity errors before the next model is tried. Set `GEMINI_DISCOVER_MODELS=false` to disable discovery, or set `GEMINI_MODELS` as a comma-separated priority list. `GEMINI_MODEL` remains supported for a single-model override.
